        ...
        <div className="space-y-4">
          {followers.map(follower => (
            <div key={follower.id} className="bg-white rounded-lg shadow p-4 flex items-center justify-between">
              <Link to={`/profile/${follower.id}`} className="flex items-center gap-4">
                <div className="h-12 w-12 rounded-full bg-primary-100 flex items-center justify-center">
                  {follower.profilePhotoUrl ? (
                    <img 
                      src={"http://localhost:8081" + follower.profilePhotoUrl} 
                ...
                <div>
                  <h3 className="font-medium text-gray-900">{follower.name || 'Unknown'}</h3>
